...
export default function EcosystemSimulator({ onOpenPlanner }) {
  ...
  return (
    <section id="ecosystem" className="py-24 relative overflow-hidden bg-[#070b12] border-t border-b border-white/5 cyber-grid-bg z-10">
      
      {/* Background Ambience */}
      <div className="absolute top-1/3 left-1/4 w-80 h-80 bg-cyan-500/10 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div className="container-custom relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white tracking-tight mb-4">
            ...
            <span className="text-gradient-cyan">Everything Under Control.</span>
          </h2>
              ...
              return (
                <button
                  ...
                  className={`cursor-pointer p-3.5 sm:p-4 rounded-xl border text-center transition-all flex flex-col items-center gap-2 select-none ${
                    isSelected
                      ? 'bg-gradient-to-b from-cyan-500/30 to-blue-500/20 border-2 border-cyan-400 text-white shadow-xl shadow-cyan-500/25 scale-[1.03]'
                      : 'bg-white/[0.04] border border-white/10 text-slate-300 hover:text-white hover:bg-white/[0.08]'
                  }`}
                >
                  <div className={`p-2 rounded-lg ${isSelected ? 'bg-cyan-400 text-black' : 'bg-white/5 text-slate-400'}`}>
        ...
        <div className="glass-panel p-6 sm:p-10 max-w-5xl mx-auto border-cyan-500/30">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {ECOSYSTEM_NODES.map((node) => {
              ...
              return (
                <div 
                  key={node.id}
                  className="p-4 rounded-xl bg-white/[0.03] border border-white/10 hover:border-cyan-500/30 transition-all flex flex-col justify-between"
                >
                  ...
                  <div className="pt-2 border-t border-white/5 flex items-center justify-between text-xs font-mono">
                    ...
                    <span className="text-cyan-300 font-semibold truncate ml-1 text-right">
                      {dynamicValue}
                    </span>
                  </div>
                </div>
